...
export const Heading1: React.FC<headingProps> = props => {
  const { content } = props;
  return <span className="text-34 mt-10 text-blue-400 font-bold md:text-42">{content}</span>;
};
...
export const Paragraph2: React.FC<paragraphProps> = props => {
  const { content } = props;
  return (
    <p className="text-14 mt-4 text-white-600 font-semibold text-start md:text-16">
      {' '}
      {content}{' '}
    </p>
  );
